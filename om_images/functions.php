...
function om_images_do_replace(&$images_array, $message, $dir_name)
{
	...
	$replace[] = 'om_images_add_link($images_array, \'img\', $dir_name, \'$1$3\')';
	$replace[] = 'om_images_add_link($images_array, \'img\', $dir_name, \'$2$4\', \'$1\')';

	if ($forum_config['o_om_images_url'] == '1') {
		$pattern[] = '#\[url\]([^\[]*?)\[/url\]#e';
		$pattern[] = '#\[url=([^\[]+?)\](.*?)\[/url\]#e';

		$replace[] = 'om_images_add_link($images_array, \'url\', $dir_name, \'$1\')';
		$replace[] = 'om_images_add_link($images_array, \'url\', $dir_name, \'$1\', \'$2\')';
	}
...
function om_images_fetch_file($path, $url) {
	...
	($hook = get_hook('om_images_fn_fetch_file_pre_check_type')) ? eval($hook) : null;
	// Now check the width, height, type
